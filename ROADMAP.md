...
> **Aktueller Stand:** `P0–P10 ✅ CODE-COMPLETE & E2E-verifiziert; Log-Rotation/Retention/
> Abfrage nachgezogen → Tag v1.0.0-rc2. Gap-Review gemacht → P11-Backlog
> (Deployment-Realität/Betrieb/Ehrlichkeit) unten, priorisiert mit Ziel+Verifikation je Punkt.
> P11.1–P11.2 ✅ (Upgrade-Restart + reconcile/restore, E2E-verifiziert). P11.3 ✅ (ECH/QUIC/DoH dokumentiert +
> mitigiert). P11.1–P11.7 ✅ KOMPLETT (E2E-verifiziert; inkl. Klassen-Lasttest 50/50). Offen NUR Human-Gates. Human-Gates: Windows-Abnahme, GPG-Key, reale AD-Fakten, Image-Publish.`
...
- **Jede Phase endet mit einem realen, bestandenen Test** (Unit lokal,
  „heavy tier" im E2E-Test). „SKIP" heißt „nicht verifiziert", nicht „ok".
...
| **P1** | Data-Plane-Image (Kerberos-SSO-MVP) | Squid-Image; Lehrer-allow/Schüler-deny **im E2E bewiesen** |
...
(lokal + Docker-E2E), und die **site-spezifischen AD-Fakten gegen eine echte
linuxmuster-7.3-Test-Umgebung verifiziert**, bevor Code darauf aufbaut.

**Deliverables:** `README.md`, `CLAUDE.md`, `docs/{architecture,decisions,threat-model,test-strategy}.md`, `.gitignore`, `LICENSE`; `scripts/tests/run.sh` (Aggregator-Skelett) + `scripts/tests/crabbox_bootstrap.sh`.
...
**E2E** beweist: Lehrer→200, Schüler→403, gesperrte Domain→403, kein
Ticket→407.
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (E2E 4/4; Commits bis `34237de`).**
...
**Definition of Done:** Auf E2E liefert
...
**Verifikation:** E2E (Summary zitieren). Empirisch klären, ob dieser
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (E2E 6/6: HTTPS erlaubt→200 gespliced,
...
**Definition of Done:** E2E erweitert: gesperrte HTTPS-Domain wird per SNI
...
**Verifikation:** E2E (HTTPS-Fälle) + `squid -k parse` mit aktiver
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (E2E 9/9 inkl. 3 Multischool-Checks; commit `0c2094e`).**
...
**Definition of Done:** E2E mit 2 Schulen × 2 Rollen: jede Instanz lässt
nur ihre Gruppe/ihr Subnetz zu; präfixierte Gruppennamen greifen; Quer-Schul-Zugriff
wird abgelehnt.

**Verifikation:** E2E (Multischool-Matrix).
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (unit 17/17 + mypy + ruff; heavy 2/2: kerberos-e2e 9/9 + echte docker-py-Container-Integration; commit `060c6cb`).**
Module modular erstellt, dann integriert. Hinweis: TLS/mTLS + `config.yml`-Secret-Generierung (`openssl rand`, `chmod 600`) landen mit dem systemd-Dienst in P9.
...
**Definition of Done:** `pytest` grün; im E2E-Test legt die API real eine
Instanz an, startet/stoppt sie (docker-py), Reconcile bringt Ist=Soll; ohne Token
→ 401, falscher Token → 403.

**Verifikation:** `run.sh unit` (pytest) lokal/CI + E2E (API steuert echten Container).
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (unit 21/21 + REALER Auto-Rollback: Update→kaputtes Image→Container-Crash→Rollback auf Known-Good, Dienst läuft weiter; commits `c159614`, `4e9caac`).**
...
**Definition of Done:** E2E: Update auf ein **absichtlich kaputtes** Image
...
**Verifikation:** E2E (Update-/Rollback-Szenario mit gutem und kaputtem Image).
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (pytest 24/24; kompletter CLI-Lifecycle gegen die API; commit `e5ed437`).**
...
im E2E-Test verifiziert.

**Verifikation:** E2E (CLI-Flow) + Unit-Tests.
...
**Verifikation:** E2E (bestehend) + Doku-Review.
...
Server-seitige Rollen-Trennung ist im E2E bewiesen; die manuelle Windows-Abnahme (DoD unten) ist ein **HUMAN-GATE** ⏸.
...
(Server-seitige Äquivalente sind im E2E bereits automatisiert.)

**Verifikation:** manuelle Abnahme auf ≥1 Windows-Client (Edge + Firefox) +
Squid-`access.log`-Kontrolle; automatisiertes Pendant im E2E.
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (build → `apt install` → systemd `active` → API `{"status":"ok"}` → CLI `health` → Upgrade 0.9.1 `active`; commit `cc9d80c`).**
...
**Definition of Done:** Auf E2E: `apt install ./linuxmuster-squid_*.deb` bringt
...
**Verifikation:** E2E (Install → API/CLI-Smoke → Upgrade → Rollback).
...
**Aufgaben:** ✅ **ABGESCHLOSSEN & E2E-verifiziert (`run.sh all`: 6 passed, 0 failed; Unit 41 + E2E 9/9 + docker-Integration + `.deb`; Security-Review mit ALLEN Befunden behoben; commits `3b4043c`, `887a291`).** Tag **v1.0.0-rc1** (volles v1.0.0 nach der manuellen Windows-Abnahme + GPG-Signierung).
...
- [x] Security-Review über den Code — **25 Befunde, alle echten behoben & verifiziert**.
...
- [x] **Alle automatisiert testbaren §0.1-Kriterien verifiziert** → Tag `v1.0.0-rc1` (volles `v1.0.0` nach den Human-Gates).

**Definition of Done:** Alle §0.1-Kriterien erfüllt und real bewiesen (Docker-E2E-
Summaries + manuelle Windows-Abnahme dokumentiert); Security-Review ohne offene
kritische Befunde; `v1.0.0` getaggt.

**Verifikation:** `run.sh all` im E2E-Test (grün) + manuelle Produktiv-Abnahme +
Review-Report.
...
**Herkunft:** Gap-Review (2026-07-03) auf `v1.0.0-rc2`. Priorisiert;
...
- [x] **Upgrade-Restart-Bug:** ✅ postinst: Frisch→`start`, Upgrade→`try-restart`; `prerm` stoppt nur bei `remove` (zweiter Bug, den der Fix aufdeckte). *Verif (Docker-E2E):* `deb_smoke` prüft **MainPID-Wechsel** über das Upgrade (9550→9939 = neu gestartet, neuer Code aktiv) + clean-slate reuse-fest. commit `12af923`.
...
**DoD:** frischer Host → Restore → laufende Instanzen, E2E-bewiesen. (Reboot selbst ist ok: `restart_policy` bringt Container zurück.)
...
- [x] **Soak/Load-Smoke** ✅ E2E-verifiziert: 50 parallele `curl --proxy-negotiate` eines Lehrers durch eine Instanz → **50/50 200, 0 5xx**. negotiate `children=20` + Gruppen-ACL halten die Klassen-Concurrency ohne Tuning. `scripts/tests/load_smoke.sh`.
