...
# in that AD group and re-checks it LIVE via LDAP (NOT from the Kerberos ticket/PAC) on a SHORT
# ttl (grace=0, no stale serving), so removing a user from the group blocks their NEW requests
# within ~10s WITHOUT a re-login. Empty -> role group only.
if [ -n "${INTERNET_GROUP}" ]; then
    ...
    INTERNET_ACL_DEF="external_acl_type inet_group ttl=10 negative_ttl=10 grace=0 %LOGIN /usr/lib/squid/ext_kerberos_ldap_group_acl -m 5 -g ${INTERNET_GROUPS_G}
