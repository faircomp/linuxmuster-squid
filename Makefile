...
# parent may receive the build results. Mount the repository's git directory as well, at its own
# path and read-only: in a git worktree (linuxmusterDEV `bin/wt`) the checkout's .git only points
# there, and without it make deb stops instead of building (in a plain clone it is the checkout's
# own .git, mounted once more):
#   GITDIR=$(git rev-parse --path-format=absolute --git-common-dir)
#   docker run --rm -v "$PWD/..":/build -v "$GITDIR:$GITDIR:ro" -w /build/$(basename "$PWD") \
...
# deleted, staged or new (not added, so not built) file. Left out on purpose: .github/ and
# .claude/ (CI and developer tooling) and dpkg-source's default ignore list (.gitignore and the
# like). Without any .git (an unpacked source package) the tree is built as it is; a .git that
# git cannot use stops the build. The build runs with a fixed PATH and none of the caller's venv,
# Python, pip, uv or git settings (packaging/clean-env.sh), whatever shell it is started from.
.PHONY: all deb clean
