...
#
# Last-Smoke: "ganze Klasse gleichzeitig" — N parallele Kerberos-Auth-Requests eines
# Lehrers durch EINE Instanz. Prüft, dass negotiate_kerberos_auth (children) +
# ext_kerberos_ldap_group_acl die Concurrency ohne 5xx bewältigen. Braucht Docker.
set -uo pipefail
...
dcx samba-tool domain exportkeytab /shared/squid.keytab --principal=squidsvc \
    || { echo "keytab-Export fehlgeschlagen"; exit 1; }
...
echo "== start origin + squid, warte healthy =="
...
[ "$ok" = 1 ] || { echo "squid nicht healthy"; $DC logs squid 2>/dev/null | tail -30; exit 1; }

echo "== LAST: $N parallele curl --proxy-negotiate (teacher1) durch squid.$DLC =="
...
fail=0
echo "== Auswertung: $okc/$totc mit 200, $s5c 5xx (N=$N) =="
# Erfolg: keine 5xx UND >=95% der Requests 200 (Kerberos-Concurrency hält).
if [ "$totc" -ge "$N" ] && [ "$s5c" = 0 ] && [ "$okc" -ge $(( N * 95 / 100 )) ]; then
    echo "  [PASS] Klassen-Last ($N gleichzeitig) ohne 5xx bewältigt"
else
    echo "  [FAIL] Last nicht sauber bewältigt (ok=$okc tot=$totc s5=$s5c)"; fail=1
    $DC logs squid 2>/dev/null | tail -20
fi
echo "== load smoke: $fail Fehler =="
exit "$fail"
