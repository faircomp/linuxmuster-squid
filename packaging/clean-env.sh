...
#    PIP_CONSTRAINT among them), GIT_*, BASH_ENV, ENV and CDPATH, and every shell function (also
#    those the caller exported);
...
#
# What it leaves to the caller on purpose: HOME, TMPDIR, the locale, proxies and CA bundles
# (HTTP(S)_PROXY, NO_PROXY, SSL_CERT_FILE, REQUESTS_CA_BUNDLE: they decide how PyPI is reached,
# the hashes still decide what is installed), DEB_* and SOURCE_DATE_EPOCH (dpkg's own knobs), and
# git's system and global configuration (make-deb.sh runs only reading git commands, with
# fsmonitor, hooks and pager off).
#
# What it cannot undo: the shell that sources it has already read the caller's BASH_ENV, imported
# the caller's exported functions (used for the lines before this file) and applied SHELLOPTS and
# BASHOPTS; LD_PRELOAD and LD_LIBRARY_PATH reach every program. Whoever sets those in the caller's
# environment runs code as the caller anyway; none of it comes from a lock.
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
for _v in $(compgen -A function); do unset -f "$_v"; done
for _v in $(compgen -e); do
    case "$_v" in
        VIRTUAL_ENV | VIRTUAL_ENV_PROMPT | CONDA_* | PYTHON* | UV_* | PIP_* | GIT_* | BASH_ENV | ENV | CDPATH)
            unset "$_v" ;;
    esac
done
unset _v CDPATH
